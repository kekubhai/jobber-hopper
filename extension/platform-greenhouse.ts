...
function getGreenhouseApplyRoot(): ParentNode {
  const selectors = [
    "#application_form",
...
function guessGreenhouseLabel(field: FormControl): string {
  const wrap = field.closest(
    ".field, .question, .demographic_question, .custom-question, [data-field-type]"
  );
  ...
  const labelEl =
    wrap.querySelector<HTMLElement>("label[for]") ??
    wrap.querySelector<HTMLElement>("label") ??
    wrap.querySelector<HTMLElement>(".label") ??
    wrap.querySelector<HTMLElement>("legend");
...
function isGreenhouseVisibleControl(field: FormControl): boolean {
...
function logGreenhouseScanHint(): void {
  console.info(
    "[Jobber Hopper] Greenhouse: scanning visible step only. After clicking Next, use popup Refresh Review or Fill again."
  );
}

...
async function refreshSession(): Promise<void> {
  try {
    ...
    if (response && typeof response === "object") {
      currentSession = response as SessionState;
    } else {
...
async function signOutFromExtension(): Promise<void> {
...
async function applyFill(): Promise<void> {
  ...
  const payload: PopupFillRequest = {
    fields: Array.from(document.querySelectorAll<HTMLInputElement>("[data-field-id][data-safe-to-fill='true']"))
      .map((input) => ({

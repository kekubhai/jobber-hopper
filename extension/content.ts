...
function injectScanJobPostButton(): void {
  const onSocial =
...
function injectAllFloatingButtons(): void {
...
function startFloatingButtonWatcher(): void {
...
function floatingButtonStyle(bottomOffset: string, background = "#111827"): string {
  return [
    "position:fixed",
    "right:20px",
    bottomOffset,
...
async function runSocialJobPostPipeline(
  button: HTMLButtonElement,
  scraped?: ScrapedSocialPost | null
): Promise<void> {
  ...
  try {
    ...
    let email: JobPostEmailDraft | null = null;
...
async function requestJobPostEmail(
  ...
  const data = (await response.json()) as { email?: JobPostEmailDraft };
  if (!data.email) {
...
function showEmailDraftOverlay(email: JobPostEmailDraft): void {
...
function overlayButtonStyle(background = "#2563eb"): string {
  return [
    "border:0",
    "border-radius:8px",
    `background:${background}`,

import type { ProfileFieldPath } from "./profile-match";

/** Rule-based match below this → request LLM schema mapping (labels only). */
export const LOW_CONFIDENCE_THRESHOLD = 0.75;

export type FormFieldLabelPayload = {
  fieldId: string;
  labelGuess: string;
  type: string;
};

export type FormFieldSchemaMapping = {
  fieldId: string;
  profileFieldPath: ProfileFieldPath | null;
  confidence: number;
};

export const PROFILE_FIELD_PATHS: ProfileFieldPath[] = [
  "personal.firstName",
  "personal.middleName",
  "personal.lastName",
...
export function isProfileFieldPath(value: string): value is ProfileFieldPath {
  return PATH_SET.has(value);
}

/** Stable JSON string for hashing — same on web and extension. */
export function canonicalFormFieldsForHash(fields: FormFieldLabelPayload[]): string {
